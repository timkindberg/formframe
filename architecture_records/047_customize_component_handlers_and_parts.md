...
**Date:** 2026-07-12
**Status:** Accepted (epic `jsonschema-form-8l8`; spike `jsonschema-form-gjq`). **Annotated by [ADR 051](./051_defaults_and_intercept.md)** — §1–§2 and §4 stand; §3 kind blankets / `r.control` as intercepts and §6 “one registrar at two scopes” do not. **Annotated by [ADR 056](./056_field_mode_is_a_recipe_result.md)** — §7’s “future rule engine” is a host recipe (field mode), not Core.
...
**Addendum (bd jsonschema-form-108 — builder-stability gaps closed):** the "read once" contract in §1 had two silent edges once `useRenderNodeRules` shipped. (a) The low-level `renderNodeRules(build)` sugar has no `useRef` of its own — calling it fresh inline (skipping the hook) rebuilds a new `RenderNode` every render with no warning at that layer. `SchemaFields` now carries its own dev-only guard on the `renderNode` prop it actually consumes, so the footgun is loud regardless of which layer produced the instability — it flags **two consecutive** identity changes (not one), so a one-off deliberate rebuild never false-positives. (b) A `useCallback(build, [deps])` whose *listed* deps update legitimately changes identity (and the hook's existing warning fires, as designed) — but a `useCallback` with an *incomplete* deps list keeps the SAME identity while its closure staleness drifts, and no runtime check can see inside a stable reference to know its insides changed. Rather than chase that undetectable case, `useRenderNodeRules` gained an explicit 3rd `deps` argument (the same shape as `useMemo`/`useEffect`): declare what should trigger a rebuild, and the resolver rebuilds on exactly that — a supported, non-silent way to have a genuinely dynamic rule *set* (not just per-render behavior, which still belongs inside a handler per §7 above).
