...
4. **Field UI subscribes per path** (`fieldModeStore.recipe.tsx`, [#181](https://github.com/timkindberg/formframe/issues/181)). Each field selects one boolean from a `useSyncExternalStore` store (`useFieldMode(s => s.hidden.has(path))`). The store notifies only when set membership changes. `useWatch` lives in the runtime, never on the component that mounts `SchemaFields`. A snapshot object in React context re-renders every field on every driver change. So does a selector-context library (use-context-selector) when the provider value is a new object. ADR 008 non-degradation applies to rule-driven hide/show, not just typing.
