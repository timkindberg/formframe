# ADR 039: The Customize Layer — Component Handlers, Arrangeable Parts, and Path-Narrowed Selectors

**Date:** 2026-07-12
**Status:** Proposed (bd `jsonschema-form-gjq` and follow-ups)
**Deciders:** Tim Kindberg
**Extends / refines:** ADR 010 (continuation primitive), ADR 013 (renderer sets / `createRenderer`), ADR 016 (render by calling, stable types), ADR 017 (component re-entry layer), ADR 012 (per-node part override typing), ADR 031 (present/render boundary), ADR 033 (schema-agnostic Core + front-ends)
...
This ADR was designed against a runnable spike (`examples/basic-react/src/App_15_React+CustomizePrototype.tsx`, bd `gjq`) that proves each decision typechecks and runs on the existing engine — `customize` is pure sugar lowering to `renderNode`, no Core change required.
...
- **Every part is render-prop-hijackable**: `<parts.X />` (default) or `<parts.X render={data => …} />`, where `data` is that part's **narrowed payload** — `Label`→`{ text, attrs, showRequired }`, `Description`→`{ text }`, `Control`→the narrowed `FieldControl` (§5), `Errors`→`ValidationIssue[]`. One uniform rule: *a part is a slot you place, optionally hijacked with a render prop that hands you its typed data.*
