...
// two IOC seams (`intercept`, `layout`) inject `{ Default, Children }`.
...
describe('component handles — <Default of/> / <Children of/>', () => {
  ...
  it('layout injects the same Default component as the import', async () => {
    const form = jsonSchemaToRuntimeTree(schema)
    const screen = await render(
      <SchemaFields
        form={form}
        layout={(root, helpers) => {
          expect(helpers.Default).toBe(Default)
          expect(helpers.Children).toBe(Children)
          return <Default of={root.children.name} />
        }}
      />
    )
    await expect
