...
- `packages/validation-*` — removed; AJV lives in `examples/.../ajvValidator.recipe.ts` (copy with JSON Schema recipes); Zod uses Core `fromStandardSchema`
...
- Customization: defaults (kind-wide) vs intercept (this node) vs `layout` (place-yourself at the root or any container — ADR 053/054). `layout` keys `root.children` off the tree’s `FormShape` (ADR 055), bound or unbound; kind-unknown handles (runtime tree, `child(path)`) expose every kind’s surface as `T | undefined` instead of forcing a guard. `useRenderNodeRules(form, rules)` still lowers to `intercept` (ADR 047/048) until the intercept bag ships. `intercept(node, { Default, Children })` is the floor (ADR 010/017); the numbered gallery apps walk up to it on purpose. `<Default of={node}/>` / `<Children of={node}/>` / `<Default of={node} parts={{…}}/>` / `<Default of={node} layout={…} />` re-enter the engine. Bare layout placements resolve through intercept (adding `parts` makes it a template call); intercept's `<Default />` is the template. A scoped `layout` callback gets a `rebind`-ed handle so `Children`/`child`/`renderItem` share the scope; reordering placements needs `key=`. The callables (`node.Default()` / `node.Resolve()` / `node.rebind()`) are the low-level primitive (ADR 016). Fractal from `<SchemaFields>` down to a single part.
