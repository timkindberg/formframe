...
**Fully fractal** — `<SchemaFields/>` resolves the form tree to its defaults. `<SchemaFields layout={(root, { Default }) => …} />` places yourself at the root ([ADR 053](./architecture_records/053_schemafields_layout_prop.md)); that is not JSX children and not `intercept`. Bound `layout` (from `useFormTree`) keys `root.children` off the tree’s `FormShape` ([ADR 055](./architecture_records/055_layout_children_formshape.md)). A bare placement inside `layout` goes through `intercept`, then the template ([ADR 054](./architecture_records/054_layout_placements_resolve.md)); adding `parts` to that placement makes it a template call instead, since an intercept that replaces the node has nowhere to apply part overrides. `<Default of={group} layout={…} intercept={…} />` is the same pair scoped to that node, and the callback receives a handle rebound to that scope, so `<Children of={group} />` resolves through it too. `SchemaFields` renders the form's *content only* — the chrome (the `<form>` element, submit, reset/cancel) is the consumer's, not a root part (ADR 013); the root's *children* are the top-level fields/groups. `intercept` without `layout` on `<Default>` is a scoped resolver — nearest scope wins. A field bottoms node-recursion (it has parts, no child nodes); an atomic part bottoms part-recursion.
