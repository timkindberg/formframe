...
- `intercept(node, { Default, Children })` — the per-node hook the renderer calls while walking the tree (historically `renderNode`). Return custom JSX to intercept a node, or `<Default of={node} />` to keep the current **defaults**. The helpers are injected (also importable).
- `<Default of={node} />` — render this node's default composition (the merged defaults table); descendants still pass through `intercept`.
...
**Fully fractal** — `<SchemaFields/>` resolves the form tree to its defaults. `<SchemaFields layout={(root, { Default }) => …} />` places yourself at the root ([ADR 053](./architecture_records/053_schemafields_layout_prop.md)); that is not JSX children and not `intercept`. `layout` keys `root.children` off the tree’s `FormShape` ([ADR 055](./architecture_records/055_layout_children_formshape.md)) — bound (`useFormTree`) or unbound (`<SchemaFields form={…}/>`, which reads the brand off `form`). Where the kind genuinely isn’t known — an unbranded runtime tree, or a dynamic `child(path)` — the handle carries every kind’s surface with `undefined` where that kind has none, so `node.parts.addButton` and `node.children?.street` need no `isArray`/`isGroup` guard, and `if (node.isArray)` still narrows. A bare placement inside `layout` goes through `intercept`, then the template ([ADR 054](./architecture_records/054_layout_placements_resolve.md)); adding `parts` to that placement makes it a template call instead, since an intercept that replaces the node has nowhere to apply part overrides. `<Default of={group} layout={…} intercept={…} />` is the same pair scoped to that node, and the callback receives a handle rebound to that scope, so `<Children of={group} />` resolves through it too. `SchemaFields` renders the form's *content only* — the chrome (the `<form>` element, submit, reset/cancel) is the consumer's, not a root part (ADR 013); the root's *children* are the top-level fields/groups. `intercept` without `layout` on `<Default>` is a scoped resolver — nearest scope wins. A field bottoms node-recursion (it has parts, no child nodes); an atomic part bottoms part-recursion.

The React **adoption** path is `useFormTree(tree, { defaults })` plus `<SchemaFields intercept={…} />` ([ADR 051](./architecture_records/051_defaults_and_intercept.md)): **defaults** are the template + parts table `<Default />` draws; **intercept** is this node (function floor, or a dotted-path map / `{ paths, where }` bag). Kind-wide look and form-lib wiring live on defaults, not on a winning registrar rule. The numbered gallery walks up to the intercept function on purpose (App_08). See the README customize section. Consumers invent capabilities on top of those foundations; FormFrame adds a generic surface only when the recipe is not canonically correct ([ADR 052](./architecture_records/052_consumers_build_on_foundations.md)).
